...
BestFS::BestFS(Graph & graph)
	:graph(graph)
{
	if (!font.loadFromFile("fonts/Bebas-Regular.ttf")) {}
	text.setFont(font);
	text.setString("Solving Best First Serach..");
...
void BestFS::SolveAlgorithm(const Location & srcpos, const Location & targetpos, const std::vector<Location>& obstacles, Grid & grid, sf::RenderWindow & createwindow)
{
	this->srcpos = srcpos;
	...
	while (!pq.empty() && !targetreached) {		//while priority queue is not empty
		...
		for (auto &neighbour : curr->neighbours) {
			...
			if (neighbour->Visited || neighbour->isObstacle) {
				...
				double estimatedgCost = nodedistance(&graph.getNode(targetpos), neighbour);
...
void BestFS::constructPath(Grid& grid)
{
...
double BestFS::nodedistance(node* a, node* b)
{
